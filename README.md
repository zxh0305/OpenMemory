...
编辑 `api/.env`，设置你的配置（AI 相关配置以 `api/.env` 为准）：
...
# OpenAI / 兼容 OpenAI API 配置（用于 AI 提取）
OPENAI_API_KEY=your_openai_api_key_here
# OPENAI_PROVIDER=openai
# OPENAI_BASE_URL=https://api.openai.com/v1
# OPENAI_MODEL=gpt-4o-mini

# Embedding 配置（可选，未配置则使用默认）
...
### 🧠 `infer=true/false` 行为说明

- `infer=true`：调用 AI 进行事实提取与重写，返回 `ai_extraction_used: true`，并在响应中包含 `extracted_text`。
- `infer=false`：不调用 AI，直接用原文写入，返回 `ai_extraction_used: false`，并提示“未启用AI提取，已存入原文”。
